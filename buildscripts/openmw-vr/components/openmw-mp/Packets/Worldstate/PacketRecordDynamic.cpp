...
void PacketRecordDynamic::Packet(RakNet::BitStream *newBitstream, bool send)
{
    ...
    else if (worldstate->recordsType == mwmp::RECORD_TYPE::POTION)
    {
        for (auto &&record : worldstate->potionRecords)
        {
            ...
            // mAutoCalc was removed; use Flags bit instead
            {
                int32_t autoCalc = send ? ((recordData.mData.mFlags & ESM::Potion::Autocalc) ? 1 : 0) : 0;
                RW(autoCalc, send);
                if (!send) {
                    if (autoCalc) recordData.mData.mFlags |= ESM::Potion::Autocalc;
                    else recordData.mData.mFlags &= ~ESM::Potion::Autocalc;
                }
            }
    ...
    else if (worldstate->recordsType == mwmp::RECORD_TYPE::MISCELLANEOUS)
    {
        for (auto &&record : worldstate->miscellaneousRecords)
        {
            ...
            RW(recordData.mData.mWeight, send);
            RW(recordData.mData.mValue, send);
            // mIsKey was removed; use Flags bit instead
            {
                int32_t isKey = send ? ((recordData.mData.mFlags & ESM::Miscellaneous::Key) ? 1 : 0) : 0;
                RW(isKey, send);
                if (!send) {
                    if (isKey) recordData.mData.mFlags |= ESM::Miscellaneous::Key;
                    else recordData.mData.mFlags &= ~ESM::Miscellaneous::Key;
                }
            }
            RW(recordData.mScript, send, true);
