...
void CellController::updateLocal(bool forceUpdate)
{
    ...
    MWWorld::World* worldImp = dynamic_cast<MWWorld::World*>(world);
    ...
    for (auto it = cellsInitialized.begin(); it != cellsInitialized.end();)
    {
        ...
        if (mpCell->getCellStore() == nullptr || mpCell->getCellStore()->getCell() == nullptr
            || worldImp == nullptr || !worldImp->getWorldScene().isCellActive(*mpCell->getCellStore()))
        {
    ...
    {
        // Compatibility no-op: Navigator no longer exposes setUpdatesEnabled.
    }
}
...
bool CellController::isActiveWorldCell(const ESM::Cell& cell)
{
    ...
    MWWorld::CellStore* store = getCellStore(cell);
    return store != nullptr && worldImp->getWorldScene().isCellActive(*store);
}
...
MWWorld::CellStore *CellController::getCellStore(const ESM::Cell& cell)
{
    ...
    if (cell.isExterior())
    {
        cellStore = &worldImp->getWorldModel().getExterior(
            ESM::ExteriorCellLocation(cell.mData.mX, cell.mData.mY, ESM::RefId()));
    }
    else
    {
        try
        {
            cellStore = &worldImp->getWorldModel().getInterior(cell.mName);
        }
        catch (std::exception&)
        {
            cellStore = nullptr;
