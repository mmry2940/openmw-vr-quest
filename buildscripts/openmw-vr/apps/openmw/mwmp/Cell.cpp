...
void Cell::updateLocal(bool forceUpdate)
{
    ...
    for (auto it = localActors.begin(); it != localActors.end();)
    {
        ...
        else
        {
            ...
            {
                if (!MWWorld::CellStore::isAccessible(actor->getPtr().getRefData(), actor->getPtr().getCellRef()))
                {
                    std::string mapIndex = it->first;
...
void Cell::initializeLocalActors()
{
    ...
    store->forEach([&](MWWorld::Ptr ptr)
    {
        ...
        if (!ptr.getRefData().isEnabled() || !MWWorld::CellStore::isAccessible(ptr.getRefData(), ptr.getCellRef()))
            return true;
