...
void LocalActor::updateCell()
{
    ...
    LOG_APPEND(TimedLog::LOG_VERBOSE, "- Moved to cell %s", ptr.getCell()->getCell()->getDescription().data());
...
void LocalActor::updateEquipment(bool forceUpdate, bool sendImmediately)
{
    ...
    for (int slot = 0; slot < MWWorld::InventoryStore::Slots; slot++)
    {
        ...
        if (it != invStore.end())
        {
            ...
            if (!::Misc::StringUtils::ciEqual(cellRef.getRefId().getRefIdString(), item.refId))
            {
                equipmentChanged = true;

                item.refId = cellRef.getRefId().getRefIdString();
...
void LocalActor::sendSpellsActiveAddition(const std::string id, bool isStackingSpell, const MWMechanics::ActiveSpells::ActiveSpellParams& params)
{
    ...
    (void)params;
    // Active-spell internals are private in this OpenMW version.
}
