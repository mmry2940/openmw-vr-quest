...
DedicatedPlayer::DedicatedPlayer(RakNet::RakNetGUID guid) : BasePlayer(guid)
{
    ...
    npc.mId = ESM::RefId();
    previousRace = npc.mRace.getRefIdString();
...
void DedicatedPlayer::update(float dt)
{
    ...
    // AI setting enum/API differs in this OpenMW version.
}
...
void DedicatedPlayer::move(float dt)
{
    ...
    if (shouldInterpolate)
    {
        ...
        world->moveObject(ptr, osg::Vec3f(lerp.x(), lerp.y(), lerp.z()));
    }
    else
...
void DedicatedPlayer::setBaseInfo()
{
    ...
    if (!RecordHelper::doesRecordIdExist<ESM::Race>(npc.mRace.getRefIdString()))
        npc.mRace = ESM::RefId::stringRefId(previousRace);

    if (!reference)
    {
        const auto* created = RecordHelper::createRecord(npc);
        if (created)
            npc.mId = created->mId;
...
void DedicatedPlayer::setEquipment()
{
    ...
    for (int slot = 0; slot < MWWorld::InventoryStore::Slots; ++slot)
    {
        ...
        if (it != invStore.end())
        {
            ptrItemId = it->getCellRef().getRefId().getRefIdString();
            ...
            if (!Misc::StringUtils::ciEqual(ptrItemId, packetRefId)) // if other item is now equipped
            {
                ...
                if (shouldRemove)
                {
                    const ESM::RefId itemId = ESM::RefId::stringRefId(ptrItemId);
                    store.remove(itemId, store.count(itemId));
                }
            }
            else
                equal = true;
        ...
        ptr.getClass().getContainerStore(ptr).add(ESM::RefId::stringRefId(packetRefId), count);
...
void DedicatedPlayer::setShapeshift()
{
    ...
    if (creatureRefId != previousCreatureRefId || displayCreatureName != previousDisplayCreatureName)
    {
        if (!creatureRefId.empty() && RecordHelper::doesRecordIdExist<ESM::Creature>(creatureRefId))
        {
            ...
            const ESM::Creature* tmpCreature = world->getStore().get<ESM::Creature>().search(ESM::RefId::stringRefId(creatureRefId));
            ...
            {
                if (const ESM::Creature* created = RecordHelper::createRecord(creature))
                {
                    creature.mId = created->mId;
...
void DedicatedPlayer::setCell()
{
    ...
    if (true)
    {
        if (!hasFinishedInitialTeleportation || Misc::StringUtils::ciEqual(getPtr().getCell()->getCell()->getRegion().getRefIdString(),
            world->getPlayerPtr().getCell()->getCell()->getRegion().getRefIdString()))
        {
            // Weather send API differs in this OpenMW version.
        }
    }
...
{
    // Speech/subtitle APIs differ in this OpenMW version.
}
...
{
    // Active-spell internals/APIs differ in this OpenMW version.
}
