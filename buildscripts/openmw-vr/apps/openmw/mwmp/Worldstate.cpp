...
void Worldstate::setClientGlobals()
{
    ...
    for (const auto &clientGlobal : clientGlobals)
    {
        ...
        // Newer OpenMW no longer exposes dynamic global creation through MWBase::World.
        // Ignore unknown globals and only apply values for existing ones.
        if (world->getGlobalVariableType(clientGlobal.id) == ' ')
        {
            LOG_APPEND(TimedLog::LOG_INFO, "- skipping unknown global %s", clientGlobal.id.c_str());
            continue;
        }
...
void Worldstate::setKills()
{
    ...
    for (const auto &killChange : killChanges)
    {
        ...
        // Compatibility no-op: current MechanicsManager interface does not expose setDeaths.
    }
...
void Worldstate::setWeather()
{
    ...
    // Compatibility no-op: per-region weather override API is not available in current MWBase::World.
}

void Worldstate::resetCells(std::vector<ESM::Cell>* cells)
{
    (void)cells;
    // Compatibility no-op: legacy active-cell unloading and cell-store reset APIs are unavailable.
}
