...
mwmp::BaseObject ObjectList::getBaseObjectFromPtr(const MWWorld::Ptr& ptr)
{
    ...
    else
    {
        ...
        baseObject.refId = ptr.getCellRef().getRefId().getRefIdString();
...
void ObjectList::addContainerItem(mwmp::BaseObject& baseObject, const MWWorld::Ptr& itemPtr, int itemCount, int actionCount)
{
    ...
    containerItem.refId = itemPtr.getCellRef().getRefId().getRefIdString();
    containerItem.count = itemCount;
    containerItem.charge = itemPtr.getCellRef().getCharge();
    containerItem.enchantmentCharge = itemPtr.getCellRef().getEnchantmentCharge();
    containerItem.soul = itemPtr.getCellRef().getSoul().getRefIdString();
...
{
    mwmp::ContainerItem containerItem;
    containerItem.refId = itemStack.mBase.getCellRef().getRefId().getRefIdString();
    containerItem.count = itemCount;
    ...
    containerItem.soul = itemStack.mBase.getCellRef().getSoul().getRefIdString();
    containerItem.actionCount = actionCount;
        ...
        if (!ptrFound.isEmpty())
        {
            ...
            for (const auto &containerItem : baseObject.containerItems)
            {
                ...
                if (action == BaseObjectList::SET || action == BaseObjectList::ADD)
                {
                    ...
                    MWWorld::ManualRef ref(MWBase::Environment::get().getWorld()->getStore(), ESM::RefId::stringRefId(containerItem.refId), 1);
                    ...
                    if (!containerItem.soul.empty())
                        newPtr.getCellRef().setSoul(ESM::RefId::stringRefId(containerItem.soul));
                ...
                else if (action == BaseObjectList::REMOVE && containerItem.actionCount > 0)
                {
                    ...
                    for (const auto itemPtr : containerStore)
                    {
                        if (Misc::StringUtils::ciEqual(itemPtr.getCellRef().getRefId().getRefIdString(), containerItem.refId))
                        {
                            if (itemPtr.getCellRef().getCharge() == containerItem.charge &&
                                ...
                                Misc::StringUtils::ciEqual(itemPtr.getCellRef().getSoul().getRefIdString(), containerItem.soul))
                            {
                                // Store the sound of the first item in a TAKE_ALL
                                if (isLocalTakeAll && takeAllSound.empty())
                                    takeAllSound = itemPtr.getClass().getUpSoundId(itemPtr).getRefIdString();
                                ...
                                if (!isLocalDrag || !isDragResolved)
                                {
                                    ...
                                    if (isLocalDrag || isLocalTakeAll)
                                    {
                                        ...
                                        *playerStore.add(itemPtr, containerItem.actionCount, false, false);
                                    }
                                }
                            }
                        }
            ...
            if (isCurrentContainer)
            {
                if (isLocalTakeAll)
                {
                    ...
                    MWBase::Environment::get().getWindowManager()->playSound(ESM::RefId::stringRefId(takeAllSound));
                }
                else
                {
                    // ContainerWindow accessor is not exposed on this WindowManager interface.
                }
            }
        }
    }
    ...
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (!ptrFound.isEmpty())
        {
            ...
            if (!activatingActorPtr.isEmpty())
            {
                ...
                {
                    std::unique_ptr<MWWorld::Action> action = ptrFound.getClass().activate(ptrFound, activatingActorPtr);
                    action->execute(activatingActorPtr);
                }
            }
        }
    }
...
void ObjectList::placeObjects(MWWorld::CellStore* cellStore)
{
    ...
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (ptrFound.isEmpty())
        {
            try
            {
            MWWorld::ManualRef ref(world->getStore(), ESM::RefId::stringRefId(baseObject.refId), 1);
                ...
                if (!baseObject.soul.empty())
                    newPtr.getCellRef().setSoul(ESM::RefId::stringRefId(baseObject.soul));
...
void ObjectList::spawnObjects(MWWorld::CellStore* cellStore)
{
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (ptrFound.isEmpty())
        {
            MWWorld::ManualRef ref(MWBase::Environment::get().getWorld()->getStore(), ESM::RefId::stringRefId(baseObject.refId), 1);
            MWWorld::Ptr newPtr = ref.getPtr();
            ...
            if (baseObject.isSummon)
            {
                ...
                if (!masterPtr.isEmpty())
                {
                    ...
                    // Summon bookkeeping APIs differ in this OpenMW version.
                }
            }
        }
        else
    ...
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (!ptrFound.isEmpty())
        {
            ...
            if (baseObject.objectState)
            {
                ...
                // a LocalActor
                const ESM::Cell esmCell = cellStore->getCell()->getEsm3();
                if (ptrFound.getClass().isActor() && mwmp::Main::get().getCellController()->hasLocalAuthority(esmCell))
                {
                    mwmp::Main::get().getCellController()->getCell(esmCell)->initializeLocalActor(ptrFound);
                }
            }
            else
...
void ObjectList::moveObjects(MWWorld::CellStore* cellStore)
{
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (!ptrFound.isEmpty())
        {
            ...
            MWBase::Environment::get().getWorld()->moveObject(ptrFound, osg::Vec3f(baseObject.position.pos[0], baseObject.position.pos[1],
...
void ObjectList::restockObjects(MWWorld::CellStore* cellStore)
{
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (!ptrFound.isEmpty())
        {
            ...
            cell = ptrFound.getCell()->getCell()->getEsm3();
    ...
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (!ptrFound.isEmpty())
        {
            ...
            if (ptrFound.isInCell()) {
                playAtPosition = Main::get().getCellController()->isSameCell(Main::get().getLocalPlayer()->cell,
                    ptrFound.getCell()->getCell()->getEsm3());
            }

            if (playAtPosition) {
                MWBase::Environment::get().getSoundManager()->playSound3D(ptrFound.getRefData().getPosition().asVec3(),
                    ESM::RefId::stringRefId(baseObject.soundId), baseObject.volume, baseObject.pitch, MWSound::Type::Sfx, MWSound::PlayMode::Normal, 0);
            }
            else {
...
void ObjectList::setDoorDestinations(MWWorld::CellStore* cellStore)
{
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (!ptrFound.isEmpty())
        {
            LOG_APPEND(TimedLog::LOG_VERBOSE, "-- Found %s %i-%i", ptrFound.getCellRef().getRefId().c_str(),
                ptrFound.getCellRef().getRefNum(), ptrFound.getCellRef().getMpNum());
            // Door destination APIs differ in this OpenMW version.
        }
    }
}
...
void ObjectList::runConsoleCommands(MWWorld::CellStore* cellStore)
{
    ...
    (void)cellStore;
    // Console pointer/command APIs differ in this OpenMW version.
}

void ObjectList::makeDialogueChoices(MWWorld::CellStore* cellStore)
{
    (void)cellStore;
    // Dialogue window/translation APIs differ in this OpenMW version.
}
...
void ObjectList::playMusic()
{
    for (const auto &baseObject : baseObjects)
    {
        ...
        MWBase::Environment::get().getSoundManager()->streamMusic(VFS::Path::Normalized(baseObject.musicFilename), MWSound::MusicType::Normal);
    }
}
...
{
    cellStore->forEach([&](MWWorld::Ptr ptr)
    {
        try
        {
...
void ObjectList::addRequestedContainers(MWWorld::CellStore* cellStore, const std::vector<BaseObject>& requestObjects)
{
    for (const auto &baseObject : requestObjects)
    {
        ...
        if (!ptrFound.isEmpty())
        {
            try
            {
                ptrFound.getClass().getContainerStore(ptrFound);
                addEntireContainer(ptrFound);
            }
            catch (...)
            {
...
{
    if (ptr.getCellRef().getRefId().getRefIdString().find("$dynamic") != std::string::npos)
    {
    ...
    baseObject.soul = ptr.getCellRef().getSoul().getRefIdString();
    baseObject.droppedByPlayer = droppedByPlayer;
    baseObject.hasContainer = ptr.getClass().hasInventoryStore(ptr);
