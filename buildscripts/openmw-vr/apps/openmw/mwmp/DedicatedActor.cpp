...
void DedicatedActor::setEquipment()
{
    ...
    for (int slot = 0; slot < MWWorld::InventoryStore::Slots; ++slot)
    {
        ...
        if (it != invStore.end())
        {
            storeRefId = it->getCellRef().getRefId().getRefIdString();
...
void DedicatedActor::setAi()
{
    ...
    LOG_APPEND(TimedLog::LOG_VERBOSE, "- actor cellRef: %s %i-%i",
        ptr.getCellRef().getRefId().getRefIdString().c_str(), ptr.getCellRef().getRefNum().mIndex, 0);
    ...
    else if (aiAction == mwmp::BaseActorList::TRAVEL)
    {
        ...
        MWMechanics::AiTravel package(aiCoordinates.pos[0], aiCoordinates.pos[1], aiCoordinates.pos[2], false, nullptr);
    ...
    else if (hasAiTarget)
    {
        ...
        if (aiTarget.isPlayer)
        {
            ...
            const std::string targetName(targetPtr.getClass().getName(targetPtr));
            LOG_APPEND(TimedLog::LOG_VERBOSE, "-- Has player target %s", targetName.c_str());
        }
        else
        {
...
bool DedicatedActor::hasItem(std::string itemId, int charge)
{
    ...
    {
        if (::Misc::StringUtils::ciEqual(itemPtr.getCellRef().getRefId().getRefIdString(), itemId) && itemPtr.getCellRef().getCharge() == charge)
            return true;
    }
...
void DedicatedActor::setPtr(const MWWorld::Ptr& newPtr)
{
    ...
    refId = ptr.getCellRef().getRefId().getRefIdString();
